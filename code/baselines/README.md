...
This folder contains the low-level reinforcement-learning algorithms used to train the thesis baselines.
...
- `wcsac_timeaware.py` trains the conservative risk-aware policy.
  - Based on the vendored WCSAC implementation in `externals/WCSAC`.
...
The baseline algorithms are TensorFlow 1.x code. PyTorch is used for the switching classifier in `code/proposed_method`, not for these low-level policies.
