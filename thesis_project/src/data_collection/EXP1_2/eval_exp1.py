...
# Budget range used during training (from wcsac_timeaware.py)
...
def make_env(budget_min: int, budget_max: int, deadline_penalty: float):
    ...
    Creates the environment matching the training setup:
      - Random robot placement (no fixed area).
      - Budget sampled randomly by the wrapper each episode.
      - eval_mode=True  → no deadline penalty applied.
      - eval_max_budget = budget_max → budget_norm identical to training.
    """
...
def rollout_collect(act_fn, env, seeds, budgets_seq, max_horizon: int, render: bool, agent_name: str):
    """
    Runs one episode per seed. The time budget B is taken from budgets_seq
    (pre-generated and shared across all agents) and set on env before reset,
    so every agent sees identical budgets for each episode.
    """
    out_rows = []
    print(len(seeds), "episodes to collect...")
    for ep_idx, s in enumerate(seeds):
        budget = int(budgets_seq[ep_idx])
        o = reset_with_seed(env, int(s))
        # reset() always overwrites env.B internally — force our shared budget after reset
        env.B = budget
        # Fix the last obs element (budget_norm), which was computed with the old B.
        # time_left_norm at t=0 is always 1.0 regardless of B, so o[-2] is already correct.
        ...
        # success = goal reached within the sampled budget B
        success = int(goal_first_step != -1 and goal_first_step <= budget)
        mean_dist_hazard = float(dist_hazard_sum / ep_len) if ep_len > 0 else float("nan")

        # print(
        #     f"Episode {ep_idx}  seed={s}  budget={budget}  ep_len={ep_len}  goal_first_step={goal_first_step}  "
        #     f"success={success}  cost_total={cum:.3f}  mean_dist_hazard={mean_dist_hazard:.3f}"
        # )
        row = {
            "agent": agent_name,
            "budget": budget,
...
def rollout_collect_switch_classifier_meta(
    classifier_model,
    meta_env,
    ...
    """Evaluate switch classifier via MetaEnv (mirrors evaluate_gap_policy)."""
    out_rows = []
    print(len(seeds), "episodes to collect (MetaEnv)...")
    p_thr = float(np.clip(switch_prob_threshold, 1e-6, 1.0 - 1e-6))
    hist_len = max(0, int(feature_history))

    for ep_idx, (seed, budget) in enumerate(zip(seeds, budgets_seq)):
...
def main():
    p = argparse.ArgumentParser(
        description=(
            "Evaluate one or more time-aware agents under the training distribution "
            "(random robot placement, random time budget sampled by the wrapper). "
            "All agents share the same episode seeds. Results are written to a single CSV."
        )
    )
    ...
    p.add_argument("--agent_names", type=str, nargs="+", default=None,
                   help="Optional display names for each agent (must match --agent_dirs in length). "
    ...
    p.add_argument("--budget_min", type=int, default=TRAIN_BUDGET_MIN,
                   help=f"Min time budget (default: {TRAIN_BUDGET_MIN}, matching training).")
    p.add_argument("--budget_max", type=int, default=TRAIN_BUDGET_MAX,
                   help=f"Max time budget (default: {TRAIN_BUDGET_MAX}, matching training).")
    p.add_argument("--max_horizon", type=int, default=0,
                   help="Max steps stored per episode in the CSV. If 0, uses budget_max.")
    p.add_argument("--render", action="store_true",
                   help="Render the environment during evaluation.")
    p.add_argument("--deadline_penalty", type=float, default=0.0,
                   help="Deadline penalty (not applied in eval_mode, kept for reference).")
    p.add_argument("--results_dir", type=str, default="results",
                   help="Directory where the output CSV is saved.")
    ...
    p.add_argument("--switch_feature_history", type=int, default=0,
                   help="Number of past feature vectors to append when evaluating the switch classifier (MetaEnv).")
    ...
    # Build list of switch thresholds (single-threshold flag kept for backward compatibility).
    ...
    # All agents evaluate on the same seeds AND same budgets for a fair comparison
    rng = np.random.RandomState(args.base_seed)
    seeds = rng.randint(0, 2**31 - 1, size=args.episodes, dtype=np.int64)
    
    budget_values = list(range(args.budget_min, args.budget_max + 1, 5))
    # Pre-generate the shared budget sequence (same RNG state for all agents)
    budget_rng = np.random.RandomState(args.base_seed + 1)
    budgets_seq = budget_rng.choice(budget_values, size=args.episodes, replace=True)
    ...
    print(
        ...
        f"  budget       ~ Uniform({{{args.budget_min}, {args.budget_min + 5}, ..., {args.budget_max}}}) "
        f"({len(budget_values)} values, step=5)  [shared sequence across agents]\n"
        ...
        f"  base_seed    : {args.base_seed}  (for generating episode seeds and budget sequence)\n"
    )
    ...
    # Output filename encodes the run (not per-agent, since all go in one CSV)
    base = (
        ...
        try:
            ...
            for thr in switch_thresholds:
                ...
                try:
                    ...
                    rows = rollout_collect_switch_classifier_meta(
                    ...
                    all_rows.extend(rows)
                    print(f"    done — {len(rows)} episodes collected (MetaEnv).")
                finally:
