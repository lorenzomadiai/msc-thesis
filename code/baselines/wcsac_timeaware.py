...
#!/usr/bin/env python
# The original WCSAC implementation is taken from the WCSAC repo and it was modified for this thesis to:
# - integrate the TimeBudgetWrapper,
# - introduce deadline-aware training,
# - support time-budget observations,
# - adapt the reward formulation to the proposed setting.

# This algorithm is used to obtain the conservative/risk-aware baseline for our experiments and it is also used as low-level policy for the proposed method.
...
def sac(env_fn, actor_fn=mlp_actor, critic_fn=mlp_critic, var_fn=mlp_var, ac_kwargs=dict(), seed=0,
        ...
        max_ep_len=1000, logger_kwargs=dict(), save_freq=15, local_update_after=int(1e3),
        ...
        budget_min=None, budget_max=None, deadline_penalty=None
        ):
    """
    ...
    if use_costs:
        if cost_constraint is None:
            ...
            cost_constraint = cost_lim * (1.0 - tf.pow(gamma, max_ep_len)) / (1.0 - gamma) / max_ep_len
    ...
    #         # Note that the episode length check is managed by the TimeBudgetWrapper, which signals done when the time limit is reached, so we only need to check for done here.
    ...
    #                 print(f"Goal met and ep_goals: {ep_goals}")
    ...
    for t in tqdm(range(total_steps // num_procs())):
        """
        ...
        # ADDED STUFF HERE
        # Step-penalty solo per StaticEnv
        # if penalty_per_step:
        #     r -= 0.2

        # print(env.unwrapped.continue_goal)
        ...
        # REMOVED BECAUSE THE TERMIONATION SIGNAL IS MANAGED BY THE TIMEWRAPPER
        ...
        # REMOVED THE CHECK ON MAX EPISODE LENGTH
        ...
        if t > 0 and t % local_steps_per_epoch == 0:
            print("Epoch completed: ", t // local_steps_per_epoch)
...
if __name__ == '__main__':
    ...
    budget_max = 220
    deadline_penalty = 1
    sac(lambda : TimeBudgetWrapper(gym.make(args.env), budget_min=budget_min, budget_max=budget_max, deadline_penalty=deadline_penalty), actor_fn=mlp_actor, critic_fn=mlp_critic,
        ...
        budget_min=budget_min, budget_max=budget_max, deadline_penalty=deadline_penalty
        )
