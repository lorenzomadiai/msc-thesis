...
#!/usr/bin/env python
# The original SAC implementation is taken from the WCSAC repo and it was modified for this thesis to:
...
# This algorithm is used firsly to obtain the aggressive/risk-seeking policy trained with a reward-only function, to compare against the proposed method and the risk-aware WCSAC baseline.
# The aggressive policy is also used as low-level policy for the proposed method, together with the conservative WCSAC policy.
# Moreover, this algorithm is also used to obtain the flat policy trained with an objective function that weighs the reward (performance)and the cost (safety) with a fixed coefficient lambda, to compare against the proposed method and the risk-aware WCSAC baseline.
