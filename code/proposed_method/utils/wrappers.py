...
class TimeBudgetWrapper(gym.Wrapper):
    """
    - Samples a time budget B at every reset
    - Appends:
        * normalized time_left in [-1, 1]
        * budget_norm = B / max_budget in [0, 1]
      as the last two elements of the observation (in this order)
    - Terminates the episode when t >= B
    - Applies a penalty ONLY if the budget expires and the goal has not been reached
    """
    ...
    def step(self, action):
        ...
        if (not done) and (self.t >= self.B):
            ...
            # penalty ONLY if the budget expires and the goal has not been reached
