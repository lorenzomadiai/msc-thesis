...
Build an offline pool of conservative-policy episodes for training and analysing
the hierarchical switching controller.

This script is used before training the switch classifier. It screens episodes
generated by the conservative policy and stores the seeds, sampled budgets, and
conservative outcome in a CSV file. For episodes where the conservative policy
fails, the script also estimates the best switching timestep k_best by comparing
the return obtained by continuing with the conservative policy against the return
obtained by switching to the aggressive policy.

The resulting pool is used to construct the supervised dataset for the proposed
hierarchical method. Successful conservative episodes provide examples where no
switch is needed, while failed conservative episodes provide candidate states
where switching may be beneficial.

The script supports two pool-construction modes:

1. Natural mode
   If --fail_frac is not provided, the pool preserves the natural success/failure
   ratio observed while screening conservative episodes.

2. Balanced mode
   If --fail_frac is provided, the script targets a specific fraction of
   conservative-fail episodes. This is useful because switch states are rarer than
   continue states, and balancing the pool helps obtain enough informative
   switching examples.

Output:
    A CSV containing:
        episode, seed, budget, cons_success, best_k,
        best_k_delta, best_k_switch_return, best_k_wait_return

    A JSON file containing summary statistics about the generated pool.

Example:

    python build_episode_pool.py \
        --cons_dir path/to/conservative_policy \
        --agg_dir path/to/aggressive_policy \
        --pool_size 1000 \
        --fail_frac 0.5 \
        --budget_min 120 \
        --budget_max 220 \
        --budget_step 5 \
        --output_csv results/episode_pool.csv
"""
